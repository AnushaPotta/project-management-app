// src/graphql/cardResolvers.js
const admin = require('firebase-admin');
const firestore = admin.firestore;

// These are separate resolvers for card operations that can be imported
// into the main resolvers.ts file

// Update a card's properties (title, description, due date, etc.)
const updateCard = async (_, { cardId, input }, { user }) => {
  if (!user) {
  ...
  try {
    ...
    // Log activity - we need the logActivity function
    try {
      // Import the logActivity function if available
